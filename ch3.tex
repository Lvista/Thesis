...
\begin{figure}[t]
	\centering
	\includegraphics[width=\linewidth]{fig/3.9-Three-state system.pdf}
	\caption{三状态系统示意图}
	\label{fig:3.9-Three-state}
\end{figure}

如上一节所述，对一个三维空间中运动的4自由物体，位置和姿态可用4个基本动作控制，理论上最少需要3个动作就可以实现在3维空间的移动。而相比双手遥控，单手遥控的可操作广度受限，所以如何设计硬件以满足输入维度就成为一个难点。
...
对这部分的具体实现将在本章\ref{ssec:simulator}节介绍。
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{fig/3.10-R_P.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.8\linewidth]{fig/3.5-circuit.pdf}
    \caption{电路示意图}
